...
public class AdmStuController {
    ...
    //获取添加数据， 并写到数据库
    @PostMapping("/add")
    public XJson doAdd(UserAccount u,
        ...
        if (myfile!=null && myfile.getSize()>0) {
            ...
            try {
                ...
                pic = largeFileService.findPic(myfile.getOriginalFilename());
            } catch (IOException e) {
    ...
    public XJson  doUpdate( Integer uid, String uname,String phone, String clzno,Integer xid,
                           @RequestParam(value="mypic", required = false) MultipartFile myfile) throws Exception {
