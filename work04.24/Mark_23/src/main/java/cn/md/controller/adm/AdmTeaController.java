...
public class AdmTeaController {
    ...
    //获取添加数据， 并写到数据库
    @PostMapping("/add")
    public XJson doAdd(UserAccount u , @RequestParam(value="mypic", required = false) MultipartFile myfile)  throws Exception {

        System.out.println("Add teacher=" + u);
        u.setRole("tea");
        if (myfile!=null && myfile.getSize()>0) {
            if (u.getPic() != 0 ) {
                largeFileService.delete(u.getPic());
            }
            largeFileService.add( myfile.getOriginalFilename(), myfile.getBytes());
        }
    ...
    //获取更新数据， 并写到数据库
    @PostMapping("/update")
    public XJson doUpdate(Integer uid, String uname,String phone,Integer xid,
        ...
        if (myfile!=null && myfile.getSize()>0) {
            try {
                if (user.getPic() != 0 ) {
                    largeFileService.delete(user.getPic());
                }
                ...
                int pic = largeFileService.findPic(picName);
                user.setPic(pic);
            } catch (IOException e) {
